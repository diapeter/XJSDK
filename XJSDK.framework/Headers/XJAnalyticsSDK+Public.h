 ...
 *
 * @discussion
 * 使用 XJAnalyticsSDK 类来跟踪用户行为，并且把数据发给所指定的 SensorsAnalytics 的服务。
 * 它也提供了一个 SensorsAnalyticsPeople 类型的 property，用来访问用户 Profile 相关的 API。
 */
@interface XJAnalyticsSDK : NSObject
 ...
 */
+ (void)startWithConfigOptions:(nonnull XJConfigOptions *)configOptions NS_SWIFT_NAME(start(configOptions:));
 ...
 * @return 返回的单例
 */
+ (XJAnalyticsSDK * _Nullable)sharedInstance;
 ...
 *
 */
@property (nonatomic, getter = getMaxCacheSize) UInt64 maxCacheSize  __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 maxCacheSize")));
- (UInt64)getMaxCacheSize __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 maxCacheSize")));
...
- (void)share:(NSString *)loginId withProperties:(NSDictionary * _Nullable )properties;
 ...
 * 该功能默认关闭
 */
- (void)enableAutoTrack:(SensorsAnalyticsAutoTrackEventType)eventType __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 autoTrackEventType")));
...
- (void)enableTimeLong:(BOOL)enable;
...
@interface XJAnalyticsSDK (AppInstall)
...
@interface XJAnalyticsSDK (Deeplink)
...
#pragma mark - JSCall
@interface XJAnalyticsSDK (JSCall)
 ...
 *
 * @discussion
 * <b>请不要自己来初始化这个类.</b> 请通过 XJAnalyticsSDK 提供的 people 这个 property 来调用
 */
...
@interface XJAnalyticsSDK (Deprecated)
 ...
 */
@property (atomic) UInt64 flushInterval __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 flushInterval")));
 ...
 */
@property (atomic) UInt64 flushBulkSize __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 flushBulkSize")));
 ...
 * 默认值为 YES
 */
@property (atomic) BOOL flushBeforeEnterBackground __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 flushBeforeEnterBackground")));

/**
 * @abstract
 * 根据传入的配置，初始化并返回一个 XJAnalyticsSDK 的单例
 *
 @param configOptions 参数配置
 @return 返回的单例
 */
+ (XJAnalyticsSDK *)sharedInstanceWithConfig:(nonnull XJConfigOptions *)configOptions __attribute__((deprecated("已过时，请使用 + (void)startWithConfigOptions: 方法进行初始化")));
 ...
 * @return 返回的单例
 */
+ (XJAnalyticsSDK *)sharedInstanceWithServerURL:(nullable NSString *)serverURL
 ...
 * @return 返回的单例
 */
+ (XJAnalyticsSDK *)sharedInstanceWithServerURL:(nonnull NSString *)serverURL
 ...
 * 该功能默认关闭
 */
- (void)enableAutoTrack __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 autoTrackEventType")));
 ...
 */
- (void)enableVisualizedAutoTrack __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 enableVisualizedAutoTrack")));

/**
 开启 HeatMap，$AppClick 事件将会采集控件的 viewPath
 */
- (void)enableHeatMap __attribute__((deprecated("已过时，请参考 XJConfigOptions 类的 enableHeatMap")));
